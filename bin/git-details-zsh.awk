...
END {
    ...
    output = bright_green "±" endc "[ ";
    if(repo) {

        if(bareRepo ~ "true") {
            output = output bright_cyan "bare repository";
        } else {
            output = output bright_cyan branch;
            ...
            if(changes["staged"] > 0 || changes["unstaged"] > 0 \
               || changes["untracked"] > 0 || changes["unmerged"] > 0) {
                ...
                output = output " " bright_green changes["staged"] endc separator;
                output = output bright_yellow changes["unstaged"] endc separator;
                output = output violet changes["unmerged"] endc separator;
                output = output bright_red changes["untracked"] endc;

            }

            if(stashCount > 0) {
                output = output " ⥃ " bright_yellow stashCount endc;
            }
        }
